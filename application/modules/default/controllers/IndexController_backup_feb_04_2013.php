...
class IndexController extends Zend_Controller_Action {
	...
	public function indexAction() {
		...
		$this->view->headTitle()->append('Web Hositng , Web Design & Development');
		...
		if ($this->_request->isPost()) {
			...
			if (!empty($domain)) {
				...
				try {
					...
					$domains_prices[$dm] = isset($allDomainPrices[$extension][1]) ? $allDomainPrices[$extension][1] : '';
					...
					foreach ($extensions as $value) {
						if($value !== $extension) {
							...
							$domains_prices[$dm] = isset($allDomainPrices[$value][1]) ? $allDomainPrices[$value][1] : '';
						}    
					}
					$this->view->suggested_result = $client->CheckAvailability($suggested_list);
					$this->view->rs_search = $this->wwd_session->rs_domain;
